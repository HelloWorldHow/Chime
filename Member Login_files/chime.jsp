...
if (typeof usi_analytics === 'undefined') {
	usi_analytics = {
		...
		usi_app.main = function () {
			...
			if (location.href.toLowerCase().indexOf("cjevent") != -1) {
				usi_commons.load_script("https://www.upsellit.com/launch/blank.jsp?chime_click=" + encodeURIComponent(location.href));
				if (usi_commons.gup("SID") == "chat") {
					if (usi_commons.gup("CJEVENT") != "") {
						usi_cookies.create_cookie("cjevent", usi_commons.gup("CJEVENT"), 30 * 24 * 60 * 60);
					}
					if (usi_commons.gup("cjevent") != "") {
						usi_cookies.create_cookie("cjevent", usi_commons.gup("cjevent"), 30 * 24 * 60 * 60);
					}
		...
		usi_app.load = function () {
			...
			if (location.pathname != "/" && '/apply-credit-af/|/apply-debit-af/|/apply-debit-af/|/apply-hys-af/|/apply-spotme-af/|/applycj-spotme100-sym2020/|/applycj-hys/|/applycj-cmb-nhfgpe-sym2020/|/applycj-gpe-sym2020/|/applycj-cb/|/applytp2/|/applytp-gpe-sym2020/|/applytp3/|/applytp-hys2/|/applytp-spotme100-sym2021/|/applytp-cb/|/applyafhys/'.indexOf(location.pathname) != -1) {
				usi_commons.load_view("p5Lh7I1HEVO9ZA4jqECU5xU", "42016", usi_commons.device);
			}
		};

		usi_app.monitor_for_receipt = function() {
			try {
				if (location.href.indexOf("/enroll/#/success") != -1) {
					usi_commons.load_script("https://www.upsellit.com/active/chime_pixel.jsp");
				} else {
		...
		usi_app.monitor_for_analytics = function() {
			try {
				if (typeof(usi_app.last_url) === "undefined" || usi_app.last_url != location.href) {
					...
					//usi_analytics.send_page_hit("VIEW", "8415");
				}
